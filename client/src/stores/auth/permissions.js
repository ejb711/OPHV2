...
export function createPermissionHandlers({ role, isOwner, effectivePermissions, rolePermissions }) {
  ...
  async function fetchRolePermissions(roleName) {
    try {
      console.log(`[auth] Fetching permissions for role: ${roleName}`)
      ...
      if (roleDoc.exists()) {
        ...
        console.log(`[auth] Loaded ${permissions.length} permissions for role ${roleName}`)
      } else {
        console.warn(`[auth] Role document not found: ${roleName}`)
        rolePermissions.value = new Set()
      }
    } catch (err) {
      console.error('[auth] Error fetching role permissions:', err)
      rolePermissions.value = new Set()
    }
  }
