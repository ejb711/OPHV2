  ...
  async function fetchUserPermissions(uid) {
    ...
    try {
      ...
      if (userDoc.exists()) {
        ...
        console.warn('[auth] User document not found:', uid)
        role.value = 'pending'
        userDocument.value = null
      }
    } catch (err) {
      console.error('[auth] Error fetching user permissions:', err)
